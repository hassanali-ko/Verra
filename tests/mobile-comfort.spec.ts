...
test('mobile controls stay reachable; calm settings persist without changing an arrangement',async({page})=>{
 ...
 await page.getByRole('button',{name:'Review next step',exact:true}).click();await expect(page.getByRole('button',{name:'Approve question'})).toBeInViewport();
 ...
 await page.getByRole('button',{name:'Accessibility and reading preferences',exact:true}).click();const dialog=page.getByRole('dialog');await expect(dialog).toBeVisible();await page.getByLabel('Quiet layout',{exact:false}).uncheck();await expect(page.locator('html')).toHaveAttribute('data-calm','off');await expect(page.locator('html')).toHaveAttribute('data-reading','comfortable');await page.keyboard.press('Escape');await expect(dialog).not.toBeVisible();await expect(page.getByRole('button',{name:'Accessibility and reading preferences',exact:true})).toBeFocused();
 ...
 await page.getByLabel('A name for this plan').fill('A comfortable library visit');await page.getByLabel('Venue name').fill('Example library');await save.click();await expect(page.getByRole('heading',{name:'A comfortable library visit',exact:true})).toBeVisible();
});
test('calm mode is readable in both themes on narrow phones and desktop',async({page})=>{
 for(const width of [1440,390,320])for(const theme of ['light','dark']){
  ...
  await page.getByRole('button',{name:'Accessibility and reading preferences',exact:true}).click();expect((await new AxeBuilder({page}).withTags(['wcag2a','wcag2aa','wcag21aa']).analyze()).violations).toEqual([]);await page.keyboard.press('Escape');
 }
});
