...
test('landing leads into an account-free judge flow with scoped decisions and persistent samples',async({page})=>{
 ...
 await page.getByRole('button',{name:'Access needs',exact:true}).last().click();await page.getByLabel('May be shared in this sample arrangement').first().uncheck();await page.getByRole('button',{name:'Approve question'}).click();await expect(page.getByRole('status')).toContainText('Choose which needs may be shared');await page.getByLabel('May be shared in this sample arrangement').first().check();
