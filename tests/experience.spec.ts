...
test('landing leads into an account-free judge flow with scoped decisions and persistent samples',async({page})=>{
 ...
 await page.getByRole('button',{name:'Access needs',exact:true}).last().click();await page.getByLabel('May be shared in this sample arrangement').first().uncheck();await expect(page.getByRole('button',{name:'Approve sample question'})).toBeDisabled();await page.getByLabel('May be shared in this sample arrangement').first().check();
 await page.getByRole('button',{name:'Approve sample question'}).click();await expect(page.getByRole('status')).toContainText('No message was sent');
 await page.getByRole('button',{name:'Pause',exact:true}).click();await expect(page.getByRole('button',{name:'Load sample venue reply'})).toHaveCount(0);await page.getByRole('button',{name:'Resume',exact:true}).click();await page.getByRole('button',{name:'Load sample venue reply'}).click();
 await page.getByRole('button',{name:'Review visit plan'}).click();await expect(page.getByRole('heading',{name:'Your sample visit plan'})).toBeVisible();await expect(page.getByText('A quieter arrival time is still unconfirmed.',{exact:false})).toBeVisible();
 await page.reload();await expect(page.getByRole('heading',{name:'Your sample visit plan'})).toBeVisible();
...
test('sample creation, reusable needs, filter, decline and reset all work',async({page})=>{
 ...
 await page.getByRole('button',{name:'Your visits',exact:true}).click();await page.getByRole('button',{name:'Arrange a visit',exact:true}).click();await page.getByLabel('A name for this plan').fill('A sample library visit');await page.getByLabel('Fictional venue').fill('Example library');await expect(page.getByLabel('What needs to work',{exact:false})).toHaveValue('A place to rest\nStep-free access');await page.getByRole('button',{name:'Save sample visit'}).click();await expect(page.getByRole('heading',{name:'A sample library visit'})).toBeVisible();await page.reload();await expect(page.getByRole('heading',{name:'A sample library visit'})).toBeVisible();
